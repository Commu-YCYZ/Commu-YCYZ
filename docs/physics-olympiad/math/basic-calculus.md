...
- 如果仅仅具有高中课内知识，则不必深究证明细节，**只需学会本文中的例子，并且会举一反三即可**。
- 遇到障碍是正常的，但不要在一个问题上纠结太久。
...
例如，$f^\prime(x)=0$ 的所有原函数为 $f(x)=C$，这种写法的意思是 $\{f(x)|f(x)=C, \forall C\in \mathbb{R}\}$，即 $C$ 是一个不定的常数（可以取任意值），所有的 $f(x)=C$ 都是 $f^\prime(x)=0$ 的原函数。
...
例如：某质点加速度 $a$ 恒等于 $0$，那么我们知道其速度 $v=v_0$，其中 $v_0$ 是一个常数。

!!! note "用微分方程表示这一过程"

    加速度为恒为 $0$，因此有

    $$\frac{\mathrm{d}v}{\mathrm{d}t}=0 \Rightarrow \mathrm{d}v=0\cdot\mathrm{d}t\Rightarrow\int\mathrm{d}v=\int0\cdot\mathrm{d}t$$

    等式两端分别是一个不定积分

    $$\begin{aligned}
    \int\mathrm{d}v&=v+C_1\\
    \int0\cdot\mathrm{d}t&=0+C_2
    \end{aligned}$$

    我们把它们写回方程，并且记 $v_0=C_2-C_1$，得到

    $$v=v_0$$

    这样，我们就完成了最简单的微分方程的求解。

    ??? info "常数与初值条件"

        计算不定积分时的常数是必须写出的，代表了某一导函数拥有一族原函数。这些原函数彼此间相差一个常数，但经过求导，它们都被归为同一个导函数，彼此间的差异被抹平了。
        
        因此，我们可以说，经过求导，原函数损失了用来区分彼此的信息。
        
        如果想要从这一族原函数中找回我们想要的原函数，就需要额外的信息（也就是把求导后丢失的信息找回来），这些信息可以被称为**初值条件**。

        回到我们的例子中，如果我们被告知该质点刚开始速度为 $v=v_0$，那么结合 $v_0=C_2-C_1$， 我们就挑出来了一个特定的原函数 $v(t)=v_0$，如此，这个质点的运动就被确定了。否则，没有初值条件，我们仍然不能完全确定所研究的系统（这一例子中是个质点）的状态。

        在以后的例子中，我们会看到，初值条件不是由方程解出的，而是由系统在某一刻的状态给出。

## 参考资料与文献

[1] (Jr.), T. G. B., Weir, M. D., Hass, J., &amp; Giordano, F. R. (2005). Thomas' calculus. Pearson.

[2]卓里奇, 蒋铎 数学分析[M]. 第2版. 版. 北京: 高等教育出版社, 2006.
